...
export default async function Page({searchParams}:{searchParams:Promise<{organisation?:string}>}){
 ...
 <section className="card section-spaced" id="demandes"><h2>Demandes d’accès <Badge>{requests.length}</Badge></h2><p>Choisissez les droits de la personne, puis validez l’envoi de son invitation. Elle ne devient membre qu’après l’avoir acceptée.</p>{!requests.length?<p>Aucune demande en attente.</p>:<ul className="item-list">{requests.map(r=><li key={r.id}><strong>{r.email}</strong><p>Reçue le {r.createdAt.toLocaleDateString('fr-FR',{timeZone:org.timezone})}</p><details><summary>Examiner et choisir les droits</summary><TeamForm organizationId={org.id} operation="approve-join" id={r.id} locked={!fresh}/></details><TeamForm organizationId={org.id} operation="reject-join" id={r.id} locked={!fresh}/></li>)}</ul>}<details><summary>Partager le code de demande d’accès</summary><p>Ce code permet uniquement de demander à rejoindre l’association. Il n’accorde aucun accès.</p><strong>{joinCode}</strong></details></section>
