...
export default async function Page({ searchParams }: { searchParams: Promise<{ organisation?: string }> }) { const { actor, db, org } = await programmeContext((await searchParams).organisation); if (!org) return <MissingSpace/>; if (!await programmeService(db).canCreate(actor, org.id)) return <section className="card"><h1>Création non autorisée.</h1><p>Demandez la permission de modifier les projets à votre administrateur.</p></section>; const suggestions=[...new Set((await programmeService(db).list(actor,org.id)).map(p=>p.title))].slice(0,30); return <BriefForm org={org} suggestions={suggestions} creationKey={randomUUID()}/>; }
