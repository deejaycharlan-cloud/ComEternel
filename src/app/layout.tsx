...
export default async function Layout({children}:Readonly<{children:React.ReactNode}>){
 ...
 if(actor?.user.emailVerified){const db=getDb(),team=teamService(db),programme=programmeService(db);const spaces=await team.listOrganizations(actor);viewer={name:actor.user.name||'Mon compte',spaces:await Promise.all(spaces.map(async s=>({id:s.id,name:s.name,deletionDate:(await db.select().from(organizationDeletions).where(eq(organizationDeletions.organizationId,s.id)))[0]?.dueAt.toISOString(),admin:s.role==='admin',canRead:(await programme.list(actor,s.id)).length>0,canCreate:await programme.canCreate(actor,s.id)})))};}
