...
export async function requestLimit(request:Request,bucket:string,max:number){await getDb().delete(requestLimits).where(lt(requestLimits.expiresAt,new Date()));const window=Math.floor(Date.now()/60000);const ip=clientIp(request.headers);const key=hash(`${bucket}:${ip}:${window}`);const [row]=await getDb().insert(requestLimits).values({key,count:1,expiresAt:new Date((window+2)*60000)}).onConflictDoUpdate({target:requestLimits.key,set:{count:sql`${requestLimits.count}+1`}}).returning();if(row.count>max)throw new ProductionError('Trop de tentatives. Patientez une minute avant de réessayer.');}
